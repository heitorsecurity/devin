...
afterRender : function(){
    ...
    if(this.pbHtml5NativeOnMobile && df.sys.isMobile && df.sys.supportHtml5Input("date")){
        ...
        df.events.addDomListener("change", this._eHtml5HiddenDate, this.html5OnChange, this);
        df.events.addDomListener("input", this._eHtml5HiddenDate, this.html5OnChange, this);
    }
...
html5OnChange : function(oEvent){
    //alert(this._eHtml5HiddenDate.value);
...
firePrompt : function(){
    ...
    if(this.isEnabled() && !this._bShowWait){
        if(this._bPickerVisible){
            this.hideDatePicker();
        }else{
            this.showDatePicker();
...
onPickBlur : function(oEvent){
    ...
    if(this._bPickerVisible){
        ...
        }, 1000);  // Incremented to support slower machines / browsers
