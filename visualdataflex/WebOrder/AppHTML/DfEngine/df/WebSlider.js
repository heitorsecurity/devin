...
afterRender : function(){
    ...
    if(window.PointerEvent){    //  Microsofts new standard adopted by W3C for touch / mouse & pen events as of IE11
        df.events.addDomListener("pointerdown", this._eControl, this.onPointerDown, this);
    }else{
        df.events.addDomListener("touchstart", this._eControl, this.onTouchDown, this);
    }
    
    df.events.addDomMouseWheelListener(this._eSliderWrp, this.onMouseWheelScroll, this);
    if(this.pbRanged){
        df.events.addDomListener("mousedown", this._eRangedSlider, this.onMouseDown, this);
    }
    df.events.addDomListener("mousedown", this._eSlider, this.onMouseDown, this);
...
*/
generateMarkers : function(bAssert){
    ...
    if(aData){
        for(i = 0; i < aData.length; i++){
            ...
            if(oLabel.piSliderValue % this.piInterval == 0 && oLabel.piSliderValue <= this.piMaxValue && oLabel.piSliderValue >= this.piMinValue){
                ...
                throw new df.Error(999, "Position {{0}} you want label '{{1}}' on is unreachable for the slider.", this, [ oLabel.piSliderValue, oLabel.psLabel ]);
            }
        }
    }
    ...
    */
    function onStopSlide(oEvent){
        ...
        if(eType === 0){
            ...
            df.events.removeDomListener("pointerup", window, onStopSlide);
            df.events.removeDomListener("pointerup", eMask, onStopSlide);
            df.events.removeDomListener("pointermove", window, onDragSlide);
        }else if(eType === 2){
            df.events.removeDomListener("touchend", window, onStopSlide);
            df.events.removeDomListener("touchend", eMask, onStopSlide);
            df.events.removeDomListener("touchmove", window, onDragSlide);
        }
    ...
    if(eType === 0){
        ...
        df.events.addDomListener("pointerup", window, onStopSlide, this);
        df.events.addDomListener("pointerup", eMask, onStopSlide, this);
        df.events.addDomListener("pointermove", window, onDragSlide, this);
    }else if(eType === 2){
        df.events.addDomListener("touchend", window, onStopSlide, this);
        df.events.addDomListener("touchend", eMask, onStopSlide, this);
        df.events.addDomListener("touchmove", window, onDragSlide, this);
    }
