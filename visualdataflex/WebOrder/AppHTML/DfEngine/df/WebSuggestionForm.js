...
initDOM : function(){
    ...
    //  Create elem
    this._eSuggestions = df.dom.create('<div class="WebSuggestions WebSug_Hidden"><div class="WebSug_Content"></div><div>');
...
suggestHide : function(){
    if(this._eSuggestions){
        ...
        df.sys.gui.removeScrollListeners(this._aScrollListeners);
...
suggestSelect : function(){
    ...
    if(this._sSelectedId){
        ...
        if(tRow){
            ...
            this.serverAction("SelectSuggestion", [ this.getServerVal() ], this.serializeVT([ tRow ]), function(oEvent){
                this.suggestHide();
            });
        }
    }
